...
export function runTestScenario(implName: string, client: { get: (path: string) => Promise<unknown>, post: (path: string, data: unknown) => Promise<unknown> }){
    it(implName, async () => {
        ...
        const batch = await client.get('/batches/allocations/'+ORDER_LINE.orderId);
