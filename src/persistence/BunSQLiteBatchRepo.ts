...
function BunSqliteBatchRepo(db: Database): BatchRepo {
	const getQuery = db.query(
		"SELECT b.id as batch_id, b.sku as sku, b.quantity, b.eta, o.order_id as order_id, o.quantity as line_quantity FROM batches as b LEFT JOIN order_lines as o ON o.batch_id = b.id WHERE id = $batchId ",
	);
	const findBatchForOrderLineQuery = db.query(
		"SELECT b.id as batch_id, b.sku as sku, b.quantity, b.eta, o.order_id as order_id, o.quantity as line_quantity FROM batches as b LEFT JOIN order_lines as o ON o.batch_id = b.id WHERE o.order_id = $orderId ",
	);
	const allQuery = db.query(
		"SELECT b.id as batch_id, b.sku as sku, b.quantity, b.eta, o.order_id as order_id, o.quantity as line_quantity FROM batches as b LEFT JOIN order_lines as o ON o.batch_id = b.id",
	);
	const insertBatch = db.query(
		"INSERT INTO batches (id, sku, quantity, eta) VALUES ($id, $sku, $quantity, $eta)",
	);
	const insertOrderLine = db.query(
		"INSERT INTO order_lines (order_id, sku, quantity, batch_id) VALUES ($order_id, $sku, $quantity, $batch_id)",
	);
