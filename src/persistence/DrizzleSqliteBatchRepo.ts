...
function DrizzleSqliteBatchRepo(bunDb: Database): BatchRepo {
	...
	const preparedInsertBatch = db
		...
		.prepare();
	const preparedInsertOrderLine = db
		.insert(orderLines)
		.values({
		...
		.prepare();
	const preparedAll = db.query.batches
		.findMany({
			with: {
				allocations: {
					columns: {
		...
		.prepare();
	const preparedGet = db.query.batches
		.findMany({
			with: {
				allocations: {
					columns: {
		...
		.prepare();
	const preparedFindBatchForOrderLine = db.query.orderLines
		.findMany({
			with: {
				allocated: {
					with: {
	...
	return {
		...
		async findBatchForOrderLine(orderId: string) {
			const line = await preparedFindBatchForOrderLine
				.execute({ orderId })
				.then((result) => result.at(0));
